...
### 2. 시각화
### 3. 모델링: 이 모델은 교통사고 발생률을 예측하기 위해 선형 회귀 분석을 사용합니다. 

1.데이터 전처리
2.데이터 스케일링: 수치형 특성(자치구별 유동인구수, 분기별 교통사고율) 들을 표준화. 각 특성의 평균이 0이고 분산이 1이 되도록 조정하여 모델이 특성 간의 단위 차이에 영향을 받지 않도록 함.
3.모델 학습
4.선형 회귀 모델 학습: 학습 데이터를 사용하여 선형 회귀 모델을 학습시킴. 이 모델은 자치구별 유동인구수/분기별 교통사고율율과 교통사고 발생률 간의 관계를 학습함.
5.모델 평가: 테스트 데이터를 사용하여 모델이 예측한 값과 실제 값을 비교하여 모델의 성능을 평가.
-R square (결정 계수): 예측 값이 종속 변수의 분산을 얼마나 잘 설명하는지 나타내는 지표. 1에 가까울수록 모델이 데이터를 잘 설명.
-RMSE (평균 제곱근 오차): 예측 값과 실제 값 사이의 차이를 측정하는 지표. 값이 작을수록 모델의 예측이 더 좋음.
6.예측 결과 시각화: 모델이 예측한 교통사고 발생률과 실제 교통사고 발생률을 시각화하여 비교하고, 예측의 정확성을 확인.
