...
- 피피티를 제작하면서 시각요소들을 사용해 그래프를 그렸고, 유동인구수가 많은곳과 적은곳 기준으로 5개 자치구만을 뽑아 시각화

##### (2) 점멸신호 운영 교차로 표시
- 기존의 엑셀에서 경위도 컬럼을 직접 추가하여 매트릭스를 완성했고, folium 라이브러리를 활용한 파이썬 코드를 짤 때 '교차로명, 경도, 위도'를 모든 행에서 하나씩 복사해가며 수기로 작성

#### 개선된 전처리 내용
##### (1) 자치구별 피해정도
- len, unique, groupby등 기본적인 파이썬 메소드를 사용해 기본적인 작업들을 훨씬 간편하게 수행
- matplotlib와 반복문을 사용해 서울시 25개 자치구 전체를 대상으로 그래프 표현

##### (2) 점멸신호 운영 교차로 표시
- 판다스 라이브러리를 활용해 해당 엑셀파일을 읽어들이고 필요한 '교차로명, 경도, 위도' 값을 데이터프레임의 컬럼값으로 불러들여 전체 코드가 매우 간결해지고 많은 데이터를 불러올 때를 대비해 효율성이 높아짐

##### 시각화 주요 개선된 사항 요약
1. 코드화 작업을 통해 효율적인 시각화 수행 가능
